...
function list(req, res, db) {
    db.all("SELECT * FROM projects", [], function(err, projects) {
        if (err) {
            console.error(err);
            res.statusCode = 500; // some kind of database error (e.g. no projects table)
...
/** @function create
  * Creates a new project and adds it to the dataabase.
  */
function create(req, res, db) {
    ...
    req.on("end", function() {
        ...
        db.run(
            "INSERT INTO projects(name, description, version, repository, license) VALUES(?,?,?,?,?)",
...
function update(req, res, db) {
    ...
    var body = "";
        ...
        req.on("end", function() {
            var project = JSON.parse(body);
            db.run(
                " UPDATE projects SET name=?, description=?, version=?, repository=?, license=? WHERE id=?",
